...
const { isOdooConfigured, findOrCreateOdooPartner, findOrCreateOdooProduct, findOrCreateGenericServiceProduct, listSaleTaxes, createDraftInvoice, fetchInvoicePdfFromOdoo, checkOdooInvoiceState } = require('../utils/odoo');
  ...
  try {
    ...
    if (!pdfBuffer) {
      const messages = {
        ...
        no_attachment_yet: 'Facture validée, mais le PDF n\'est pas encore généré côté Odoo — réessayez dans un instant.',
...
router.post('/invoices/:id/sync-status', async (req, res) => {
  ...
  try {
    ...
    if (odooState === 'cancel') {
      const releaseCancelled = db.transaction(() => {
        db.prepare('DELETE FROM invoice_orders WHERE invoice_id = ?').run(invoiceId);
        db.prepare("UPDATE invoices SET status = 'cancelled', updated_at = ? WHERE id = ?")
          .run(new Date().toISOString(), invoiceId);
      });
      releaseCancelled();
      return res.json({ ok: true, odooState, localStatus: 'cancelled', released: true });
    }
    // Facture toujours active côté Odoo (draft ou posted) — rien à
    // libérer, on renvoie juste le statut constaté pour information.
    res.json({ ok: true, odooState, localStatus: invoice.status, released: false });
  } catch (err) {
    res.status(502).json({ error: err.message });
  }
});
