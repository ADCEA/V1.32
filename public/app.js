...
function renderClientForm(existing) {
    ...
    <div class="card" style="margin-top:14px;margin-bottom:14px;background:var(--bg);">
      ...
      ${existing ? `
        <div style="border-top:1px solid var(--line);margin-top:16px;padding-top:14px;">
          <div class="collapsible-head"><h4 class="card-subhead" style="margin-bottom:4px;">Facturation Odoo</h4><button class="collapse-toggle" data-collapse-target="section-odoo-${existing.id}">▾</button></div>
          <div id="section-odoo-${existing.id}">
            <p style="font-size:11.5px;color:var(--steel);margin:0 0 10px;">Crée (ou retrouve, sans doublon) la fiche client correspondante côté Odoo.</p>
            <div id="odoo-status-${existing.id}" style="font-size:12px;color:var(--steel);margin-bottom:8px;">${existing.odooPartnerId ? `✓ Synchronisé (id Odoo ${existing.odooPartnerId})` : 'Pas encore synchronisé'}</div>
            <button class="btn-secondary" id="btn-odoo-sync-${existing.id}" style="width:auto;">Synchroniser vers Odoo</button>
            <div class="qty-save-note" id="odoo-note-${existing.id}"></div>

            <div style="border-top:1px solid var(--line);margin-top:16px;padding-top:14px;">
              <h5 style="font-size:12.5px;font-weight:600;color:var(--ink);margin:0 0 8px;">Générer une facture</h5>
              <p style="font-size:11px;color:var(--steel-light);margin:0 0 10px;">Facture créée en BROUILLON dans Odoo — rien n'est validé automatiquement, à relire et confirmer là-bas. Niveau de détail actuel : <b>${{detail:'Détail par article et par BL', per_bl:'Une ligne par BL', aggregated:'Agrégé par article'}[existing.invoiceDetailLevel || 'per_bl']}</b> (réglable ci-dessous dans Tarifs & catalogue).</p>
              <div id="invoiceable-orders-${existing.id}"><p class="empty-note">Chargement…</p></div>
              <button class="btn-primary" id="btn-generate-invoice-${existing.id}" style="width:auto;margin-top:10px;" disabled>Générer la facture</button>
              <div class="qty-save-note" id="invoice-note-${existing.id}"></div>
              <div id="past-invoices-${existing.id}" style="margin-top:14px;"></div>
            </div>
          </div>
        </div>
  ...
  if (existing) {
    ...
    document.getElementById(`btn-odoo-sync-${existing.id}`).addEventListener('click', async (e) => {
      const btn = e.currentTarget;
      const note = document.getElementById(`odoo-note-${existing.id}`);
      btn.disabled = true;
      btn.textContent = 'Synchronisation…';
      try {
        const { odooPartnerId } = await api('POST', `/api/admin/clients/${existing.id}/odoo-sync`, {});
        document.getElementById(`odoo-status-${existing.id}`).textContent = `✓ Synchronisé (id Odoo ${odooPartnerId})`;
        note.classList.remove('error');
        note.style.color = 'var(--teal)';
    ...
    renderInvoiceableOrders(existing.id);
    renderPastInvoices(existing.id);
  }
  zone.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

async function renderInvoiceableOrders(clientId) {
  const zone = document.getElementById(`invoiceable-orders-${clientId}`);
  const genBtn = document.getElementById(`btn-generate-invoice-${clientId}`);
  ...
  genBtn.onclick = async () => {
    ...
    const note = document.getElementById(`invoice-note-${clientId}`);
    genBtn.disabled = true;
...
async function renderPastInvoices(clientId) {
  const zone = document.getElementById(`past-invoices-${clientId}`);
...
const ADMIN_VIEWS = ['production', 'kanban', 'logistics', 'stats', 'historique', 'articles', 'garage', 'clients', 'settings'];
